...
where mergedata like '%set Liability_Status%' order by d.name 

-- dm_answers - gotodata
select distinct GoToData,name , questionnumber,CaseTypeGroupRef, * from dm_answers a 
inner join dm_documents d on d.code = a.documentcode 
where GoToData like '%delete from Usr_cheques_in_client_tab_srs where%' order by d.name 
...
where questiontext like '%Insert%Compass Law%' order by name,questionnumber
